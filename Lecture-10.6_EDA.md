...
```
    ...
    ## date created (size, mb): 2022-09-06 10:56:28 (8.397)

    ## file min/max dates: 1869-01-01 / 2022-09-30

    ## using cached file: ~/Library/Caches/R/noaa_ghcnd/USC00519397.dly

    ## date created (size, mb): 2022-09-06 10:56:34 (1.697)

    ## file min/max dates: 1965-01-01 / 2020-02-29

    ## using cached file: ~/Library/Caches/R/noaa_ghcnd/USS0023B17S.dly

    ## date created (size, mb): 2022-09-06 10:56:38 (0.949)

    ## file min/max dates: 1999-09-01 / 2022-09-30

`lubridate` packege help with date. `floor_date` round date down. 2.27
–\> 2.1; 3.8 –\> 3.1, etc we are doing this because we will `groupby`
month later today.
...
The output shows that there are 3\*12 groups now. Although nothing has
beed changed to the dataset.
...
`n_obs = n()` naming a new variable `n_obs` using the function `n()` to
count how many subjects there are inside each group.
...
don’t use `table()` because it is not forming a dataframe, it doesn’t
allow you to do any analysis.
...
`n_distinct()` counts how many distinct subjects there are in the
variable.
...
`tabyl` function in `janitor` package gives a quicker way to create 2x2
table.

`janitor` package is very helpful to do lots of things. count total
rows, columns, tests, etc.
...
```
    ...
    ##  5 2017-05-01      NA       28.4         NA      9.29
    ##  6 2017-06-01      23.5     18.7         27.2    8.73
    ##  7 2017-07-01      NA       12.7         NA      7.15
...
![](Lecture-10.6_EDA_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
